...
## Contributions  
1. Gesture mappings: temperature (creativity for left hand) between .65-.95, right hand openness for chord layers, real time chord generation (tried multiple iterations, but settled on root/3rd/5th/7th/9th)
...
4. Open GarageBand or equivalent > Click on empty project > select "MIDI/Software Instrument"
5. Run demo
python chopin_model.py # opens camera and sends MIDI output; remember GarageBand or equivalent must be open for the model to work 
